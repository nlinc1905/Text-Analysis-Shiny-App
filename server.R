#install.packages(c('tm', 'wordcloud', 'RColorBrewer', 'ggplot2', 'shiny', 'shinydashboard'))
library(tm)
library(wordcloud)
...
runQuery <- function(startdt, enddt) {
  ...
  #Read the data
  comms <- read.csv('nflDraftComments.csv', header=T)
  ...
  comms <- comms[comms$created_date>=startdt & comms$created_date<=enddt,]
  ...
  observe({
    if (input$execute >0) {
      ...
      output$wordFreq <- renderPlot({
        ...
        ggplot(f_word, aes(x=COMM_DATE)) + geom_point(aes(y=NUMBER_OF_COMMENTS), data=f_word, size=3, color="#56B4E9", alpha=1/2) + scale_x_date(breaks=xscale, labels=xscale) + labs(y='Number of Comments', x='Date') + ggtitle(paste("Comments Containing the Word ", "\"", input$sWord2, "\"", " Over Time", sep="")) + theme(text=element_text(size=20), plot.title=element_text(color="#666666", face="bold", size=20), axis.title=element_text(color="#666666", face="bold", size=16))
      })
